...
}neo.MapPlugin=neo.Plugin.extend({id:"Map",cssClass:"map-plugin",mapType:"WebGoogleMaps",optsEnabled:["zoom"],mapProxyUrl:builderSettings.appRoot+"map",defaultMapConfig:{zoom:15,align:"left"},template:$(".map-plugin-inner").html(),enableElement:function($el){var t=this;
$(document).ready(function(){t.loadMap($el,t.getConfig($el));
});
...
}if(!this.hasOpt("address")){$el.removeClass("show-address");
}if(!this.hasOpt("direction")){$el.removeClass("show-direction");
}neo.Plugin.bootstrap($el);
},hasOpt:function(opt){return this.optsEnabled.indexOf(opt)!==-1;
},getDefaultAddress:function(){if(!builderSettings.isBuilder){return"";
}var address=(webcom.common.UserService.getAddress()!=null)?webcom.common.UserService.getAddress():"";
if(typeof address!=="string"||address.trim().length===0){address=builderSettings.googleMapsDefaultAddress;
...
},loadMap:function($el,config){var t=this;
var loadPromise=$.Deferred();
var setupPromise=$.Deferred();
if(neo.MapPlugin.mapType==="WebGoogleMaps"){$el.find("iframe").hide();
this.setupGoogleMap($el,config,setupPromise);
}else{setupPromise.resolve(config);
}$.when(setupPromise).then(function(config){t.saveConfig($el,config);
var map=neo.webmap.create(neo.MapPlugin.mapType,config);
map.render($el.find("iframe").parent());
neo.MapPlugin.setupDirectionsButton($el,config,map);
loadPromise.resolve(map);
},loadPromise.reject);
return loadPromise;
},isMapQuest:function($el){var $iframeEl=$el.find("iframe");
return($iframeEl.size()===1&&neo.webmap.getTypeFromEl($iframeEl)==="WebMapQuest");
},setupGoogleMap:function($el,config,setupPromise){var t=this;
var locationPromise=$.Deferred();
if(config.location&&config.location.lat&&config.location.lng){locationPromise.resolve(config.location);
}else{locationPromise=neo.webmap.getLocation(neo.MapPlugin.mapType,config.address,config.apiKey);
}$.when(locationPromise).then(function(location){$.extend(config,{location:location});
if(t.isMapQuest($el)){t.replaceWithNewAssetStructure($el,config);
}setupPromise.resolve(config);
},function(status){if(builderSettings.isBuilder){console.log("Could not resolve location for address: "+config.address);
neo.Service.Logging.logApplication(4,"MapPlugin.js","setupGoogleMap","ERROR_COULD_NOT_RESOLVE_LOCATION","Could not resolve location for address: "+config.address);
}setupPromise.reject("Could not resolve location for address: "+config.address);
});
},getTemplateFromAssetConfig:function(){var mapTemplate={};
for(var i=0;
i<neo.AssetConfig.apps.length;
i++){if(neo.AssetConfig.apps[i].name==="Map"){mapTemplate=$(neo.AssetConfig.apps[i].dropHtml).find(".neo-asset-inner").html();
break;
}}return mapTemplate;
},replaceWithNewAssetStructure:function($el,config){var newTemplate={};
if(builderSettings.isBuilder){newTemplate=this.getTemplateFromAssetConfig();
}else{newTemplate='<div class="neo-map-wrapper">        <div class="mapAddressShow">          <span class="mapAddress"></span>        </div>        <div class="neo-mapquest">          <iframe class="mapiframe" src=""></iframe>        </div>        <div class="mapDirection" align="left">          <button type="button" onclick="window.open()">Get Direction</button>        </div>      </div>';
}var $template=$(newTemplate);
$template.find("span.mapAddress").text(config.address);
$el.find(".neo-asset-inner").html($template);
if(builderSettings.isBuilder){neo.savePage();
}},saveConfig:function($el,config){if(builderSettings.isBuilder){$el.attr("data-map-config",JSON.stringify(config));
}},getConfig:function($el){var config=$.extend(true,{},this.defaultMapConfig,{address:this.getDefaultAddress(),align:$el.find(".mapDirection").attr("align"),});
if($el.attr("data-map-config")){config=JSON.parse($el.attr("data-map-config"));
}else{if(this.isMapQuest($el)){$.extend(config,neo.webmap.getConfigFromEl($el.find("iframe")));
}}if(neo.MapPlugin.mapType=="WebGoogleMaps"){$.extend(config,{renderWith:"jsApi",proxy:{url:neo.MapPlugin.mapProxyUrl,params:{vanityDomain:builderSettings.isBuilder?userSettingsService.domain:builderSettings.siteDomain}}});
}else{config.proxy=false;
}return config;
},setupDirectionsButton:function($el,config,map){var $directionsEl=$el.find(".mapDirection");
var directionsUrl="";
var directionsPromise=$.Deferred();
if(neo.MapPlugin.mapType==="WebGoogleMaps"){var location=config.location;
directionsUrl="https://www.google.com/maps/dir//"+location.lat+","+location.lng+"/@"+location.lat+","+location.lng;
directionsPromise.resolve(directionsUrl);
}else{directionsPromise=map.getDirectionsUrl();
}$.when(directionsPromise).done(function(directionsUrl){var onClickJsCode="window.open('"+directionsUrl+"')";
$directionsEl.find("button").attr("onclick",onClickJsCode);
});
}});
